...
namespace BAC.Seguridad.Autenticacion
{
    ...
    public class RestToken
    {
        ...
        /// <returns></returns>
        public static string GetResponse_POST(string url, string data, string tokenAnterior)
        {
            int eventos = 0;
            ...
            do
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(tokenAnterior))
                    {
                        wr.Headers.Add("Authorization", "Bearer " + tokenAnterior);
                    } 
