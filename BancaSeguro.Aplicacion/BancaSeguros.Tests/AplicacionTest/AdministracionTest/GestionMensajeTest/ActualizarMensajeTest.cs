namespace BancaSeguros.Tests.AplicacionTest.AdministracionTest.GestionMensajeTest
{
    ...
    [TestClass]
    public class ActualizarMensajeTest
    {
        [TestMethod]
        public void ActualizarGeneraExcepcion_RetornaResultadoConError()
        {
            ...
            Mensaje mensaje = new Mensaje { DescripcionMensaje = "Descripcion personalizada error", IdEvento = 1, Llave = "ActualizarMensaje" };

            mockRepositorioMensajes.Setup(x => x.ActualizarMensaje(objMensaje)).Throws(excepcion);
            mockRepositorioMensajes.Setup(x => x.ConsultarMensajePorId(22, "AM-001")).Returns(objMensaje);
            ...
            //act
            Resultado resultado = objGestion.ActualizarMensaje(objMensaje);
            string mensajeEsperado = "0 AM-001 ";
        ...
        [TestMethod]
        public void ActualizaretornaError_RetornaMensajeVacio()
        {
            //arrange
            Mensaje objMensaje = new Mensaje();
            ...
            mockRepositorioMensajes.Setup(x => x.ActualizarMensaje(objMensaje)).Returns(resultadoInsercion);
            BancaSeguros.Aplicacion.Administracion.GestionMensaje objGestion = new BancaSeguros.Aplicacion.Administracion.GestionMensaje(mockRepositorioMensajes.Object);
        ...
        public void InsertaExitosamente_RetornaMensajePersonalizado()
        {
            ...
            Mensaje mensaje = new Mensaje { DescripcionMensaje = "Descripcion personalizada insercion", IdEvento = 1, Llave = "ActualizarMensaje" };

            mockRepositorioMensajes.Setup(x => x.ActualizarMensaje(objMensaje)).Returns(resultadoInsercion);
            mockRepositorioMensajes.Setup(x => x.ConsultarMensajePorId(22, "AM-002")).Returns(objMensaje);
            BancaSeguros.Aplicacion.Administracion.GestionMensaje objGestion = new BancaSeguros.Aplicacion.Administracion.GestionMensaje(mockRepositorioMensajes.Object);

            //act
            Resultado resultado = objGestion.ActualizarMensaje(objMensaje);
            string mensajeEsperado = "AM-002 ";
